...
void I_InitJoystick(void)
{
    ...
    joybfire   = JOY_BTN_R_LEFT;
    joybstrafe = -1;
    joybuse    = JOY_BTN_R_UP;
    joybspeed  = JOY_BTN_R_DOWN;
    joybjump   = -1;

    joybstrafeleft = -1;
    joybstraferight = -1;
    joybprevweapon = -1;
    joybnextweapon = JOY_BTN_R_LEFT;
    joybmenu    = JOY_BTN_START;
    joybautomap = JOY_BTN_RB;
...
static int GetAxisState(int axis, int invert)
{
  ...
  if (axis == 0)
  {
    ...
    joy_x_data = joy_msg.l_x - joy_x_offset;


    joy_x_out = map(joy_x_data, -128, 127, -32767, +32767);
  ...
  if (axis == 1)
  {
    ...
    joy_y_data = joy_msg.l_y - joy_y_offset;


    joy_y_out = map(joy_y_data, -128, 127, -32767, +32767);
